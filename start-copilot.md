...
| 12 | `12-feedback` | Feedback | Wrap-up | 10 | Points user to `feedback-prompt.md`. The prompt file instructs Copilot to conduct a structured interview (one question at a time): best moment, what didn't work, missing topics, difficulty level, daily use intention. After all answers: summarize as Markdown, ask for name/initials, then ask if they have a GitHub account. **If yes:** generate a pre-filled GitHub Issue URL (`https://github.com/abatplus/discover-ai-with-ai/issues/new?title=Feedback%3A%20<name>&body=<url-encoded-feedback>`) so the user just clicks and submits. **If no:** save as `sessions/12-feedback/feedback.md` and tell the user to send the file via email to the address provided by the workshop organizers. |
